...
// export default function Fachschaften({fachschaften}: {fachschaften: any[]}) {
        ...
        <div className="flex flex-row">
            ...
            <div className="flex-initial w-1/4 flex-col gap-4 hidden md:flex">
                ...
                {fachschaften && fachschaften.map((entry) => (
                    // <button key={entry.data().name} className={`mx-auto font-bold text-xl hover:scale-125 transition-all ${entry.id == current_id ? 'text-blue-500 scale-150 hover:scale-150' : ''}`} onClick={() => setCurrent(entry.id)}>
                    //     {entry.data().name}
                    // </button>
                    <FachschaftNavEntry key={entry.id} entry={entry} setCurrent={setCurrent} currentId={current_id} />
                ))}
            </div>
